...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--pt", help="Assignment/scoring partition date in YYYYMMDD format.")
    parser.add_argument("--assignment_start_date", help="Legacy alias for the assignment/scoring partition date in YYYYMMDD format.")
    parser.add_argument("--assignment_end_date", help="Legacy alias for the assignment/scoring partition date in YYYYMMDD format.")
    parser.add_argument("--reference-pt", "--reference_pt", dest="reference_pt", help="Optional explicit reference prediction partition for daily drift checks.")
...
def _resolve_current_pt(args: argparse.Namespace) -> str:
    if args.pt:
        datetime.strptime(args.pt, "%Y%m%d")
        if args.assignment_start_date and args.assignment_start_date != args.pt:
            raise ValueError(
                f"`--pt` and `--assignment_start_date` disagree: pt={args.pt} assignment_start_date={args.assignment_start_date}"
            )
        if args.assignment_end_date and args.assignment_end_date != args.pt:
            raise ValueError(
                f"`--pt` and `--assignment_end_date` disagree: pt={args.pt} assignment_end_date={args.assignment_end_date}"
            )
        return str(args.pt)
    if not args.assignment_start_date or not args.assignment_end_date:
        raise ValueError("Response monitoring requires `--pt` or both legacy assignment date arguments.")
    return _validate_assignment_dates(args.assignment_start_date, args.assignment_end_date)
...
def main() -> None:
    ...
    current_pt = _resolve_current_pt(args)
